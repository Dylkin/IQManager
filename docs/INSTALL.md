...
1. Установите [JDK 25](https://www.oracle.com/java/technologies/downloads/#java25) и [Maven 3.9+](https://maven.apache.org/download.cgi)
2. Распакуйте архив `tenderbot-project.zip`
3. Откройте **PowerShell** в папке проекта
4. Выполните:

```powershell
# Разрешить выполнение скриптов (однократно, если запрещено)
...
6. Дождитесь сборки и откройте: **http://localhost:8080**

### Вариант 2: Быстрый запуск (минимум ввода)

```powershell
Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope Process
.\scripts\start-quick.ps1
```

### Вариант 3: CMD (стандартный)

Откройте папку `scripts/` и запустите `start-with-config.bat` — откроется CMD с диалоговым вводом.
